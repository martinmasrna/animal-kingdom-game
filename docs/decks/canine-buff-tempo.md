...
| Name | Tag | Str | Effect |
|---|---|---:|---|
| **Dhole** | Canine | 3 | Battlecry: give all adjacent Canines +2 strength. |
| **Bush Dog** | Canine | 3 | Once a turn, when this gains strength, give all friendly adjacent Canines +1 strength. |
| **Jackal** | Canine | 3 | Whenever an adjacent unit is removed, gain 3 food. |
...
| **Fox** | Canine | 5 | Whenever this gains strength, draw a card (once per turn). *(gas engine)* |
...
- **⚠⚠ Persistent strength buffs ("+X strength") — a whole new subsystem.** Three distinct flavors that must be modeled separately:
  ...
  2. **Permanent counters (one-time, persist after the granter leaves):** Dhole "+2 to adjacent Canines", Howl "+1 to all other Canines", end-of-turn "+1 to adjacent Canine", hellhound's returned Canine "+2". These attach to the *unit instance* and stay.
...
- **Food on adjacent removal** (Jackal): any adjacent unit removed → +3 food (friend or enemy — confirm).
