...
- [x] **(Decision G) Once-per-turn caps on value/food triggers — RULED 2026-07-02: all uncapped, as printed.** Martin: "that's how the cards were designed." Applies to the reviewed triggers — **Queen Adira** (draw per Cat-kill), **King Theron** (remove per Cat-cover) [Cats]; **Eon** (+1 food per draw/shuffle/remove), **Vulture** (+5/remove), **Egg Eater** (+10/egg-removed) [Egg]; **Queen Honoria** (+5/Colony played), **Falstaff** (+3 per food gain) [Colony]; **Jackal** (+3/adjacent removal) [Canine]. Rattlesnake has since changed to uncapped +1 strength per own shuffle wherever it is and no longer uses a cap dial. Every remaining `cap_*` flag in `engine/config.py` stays present and defaults `False`; the dials are kept as sim what-if tooling per the original "flag as sim dials" ask, but the shipped/default behavior for every trigger is the printed, uncapped text. Tests: `tests/test_decision_g_caps.py`.
- [ ] **(Decision H) Re-derive the whole food economy for the reworked pool — IN PROGRESS, first two rulings shipped 2026-07-02.** Balance all the new numbers on one shared scale against `win_food` (100) and region output (`maps.json`), **replacing the legacy v0 set below** (which references retired cards).
  - [x] **Methuselah** (ramp, end-of-turn passive): **10 → 5**. Was 2x any other recurring passive in the pool (Worker Wasp is 3/turn); a str-5 Immovable body producing that much unconditionally, forever, once placed, was an outlier under any reading of the shared scale.
  - [x] **food_otk's guaranteed one-off floor: doubled.** The deck's unconditional food package (Chipmunk + Squirrel + Flying Squirrel, one copy each) summed to only ~20 against the v0 design target of an accelerant reaching *roughly half of win_food* (~50) — and food_otk is the confirmed-weakest deck by a wide margin in both greedy and referee sims (see the finding above). Ruled: **Chipmunk** 5+5 → **10+10**, **Squirrel** 6 → **12**, **Flying Squirrel** 4 → **8**, **Gazelle** (deathrattle) 20 → **40**. Updated in `engine/config.py`, `data/cards.json` (printed text), and `docs/decks/food-otk.md`/`ramp.md`.
    - ⚠ **Follow-up, not yet resolved:** Carmilla sac'ing 3 Gazelles now nets **120 food** in one Battlecry (was 60) — clears `win_food` (100) before Scrooge's doubler even applies. Flagged for a look once sim data comes in on whether the OTK is now *too* consistent/early, not walked back pre-emptively.
  - [ ] **Still open** — the 20-cost bodies (Elephant/Borealis/Aquila/Bulwark, a pacing question distinct from the recurring-engine numbers) + **Fig Tree** (+20) / **Watering Hole** Landmarks; **Egg Eater** 10 / **Vulture** 5 / **Eon** 1-per-event [Egg]; **Queen Marabunta** (+4/Colony) / **Queen Honoria** (+5/play) / **Worker Ant** (+8) / **Worker Bee** (5+5) / **Falstaff** (+3 rider) [Colony]; **Scrooge** store→2× window. These were checked against the shared scale and did not stand out as outliers in the initial pass (see the strategic-advisor review, 2026-07-02) — revisit if sim data or further play says otherwise. Keep every number a placeholder in `engine/config.py` — tune there, never hard-code in effect logic.
