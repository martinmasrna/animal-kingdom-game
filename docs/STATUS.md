...
4. **Egg Control was redesigned on 2026-09-26 and is untested.** It won 13% (referee matrix): Eggs were a slower Draw once Draw drew 2, and in 32 played games (Martin 6–4 at the web client, Claude 1–19 in simuls) every loss came from strength 7–8 units it couldn't answer. Now Snake/Bird control: Eggs and Egg Eater cut, Aurum a Bird, King Cobra (any-size removal after a shuffle), Puff Adder (removes its coverer while you control a Bird) and Secretary Bird (draws a Snake) added. Next: a fast-bot deck check, Claude's ten-game simul, then Martin's games with live commentary. Details in [`cards/decks/egg-control.md`](cards/decks/egg-control.md). Goodstuff still beats the field 80% under RefereeBot. Data in [`balance.md`](balance.md).
