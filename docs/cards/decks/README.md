...
deferred — see `docs/STATUS.md` and the per-deck legendary headers; 2–3 alternates each in `flavor-review.md`
...
| 4 | Ramp | `ramp.md` | ramp food → huge `Costs 15` bodies | ✅ all 14 defined; legendary names **provisional** (Methuselah, Borealis, Aquila, Bulwark — 2026-06-30) |
...
- **C. Card types beyond animal units. — RESOLVED (2026-06-28), revisit later.** `cards.md` §2 relaxed to permit **non-animal cards, used sparingly** (a deliberate minority so the "Animal Kingdom" identity holds). Card model = **three flavor categories, two engine behaviors:**
  ...
  - **TODO — revisit:** sanity-check this once more before building `cards.json`/rules (see `docs/STATUS.md`) — confirm Egg-as-unit doesn't create odd interactions and that "occupant" vocabulary reads cleanly across all card text.
- **D. Keyword registry — RESOLVED (2026-06-28).** Canonical definitions now live in **`docs/rules/keywords.md`** (single source of truth — don't re-explain keywords in deck/card files). Summary: official keywords are **Flight, Immovable, Fragile, Apex Predator, Battlecry, Deathrattle**; `Costs X food` is a placement *cost*, not a keyword (→ F). **Apex Predator** is a keyword, **not** vanilla (Tiger/Anaconda/Polar Bear + giant legendaries) and is now fully specified (normal covering strength rules; may eat your own units; can't be placed on an HQ; destroys Eggs/Landmarks). **Battlecry/Deathrattle** stay placeholder names (rename deferred, `docs/STATUS.md`); Deathrattle prints as "Deathrattle: …". Only **Lion** (Cats) is a true vanilla body.
...
- **F. New events/ops — RESOLVED (2026-06-28, quickfire round).**
  ...
  - **⚠ Remove Pile model (emerged from F5):** there is **one shared Remove Pile** — no separate "discard pile." **"Remove"** = any card sent there (from hand, deck, or board). **Two trigger tiers:** a **remove trigger** (fires on *any* removal) vs a **Deathrattle** (the narrower case of a *unit leaving the board*). *Reconciliation DONE (2026-07-15): `overview.md` §3.7 is **Remove Pile**; `cards.json` never used "discard"; the deck docs + `card-candidates.md` are swept. Only `cards.md` still says "discard" — left alone deliberately, as it's stale 0.0.1 content, not a wording problem (see `../backlog.md`).*
...
- **H. Food economy re-tuning.** New numbers everywhere (the costed bodies — **15**, not 20: `a27f0df` cut them and this line was never updated — 20-food Landmarks, Gazelle +20, Egg Eater 10…) vs `win_food` 100 and region 10/20. The whole shared scale needs re-derivation in sim.
...
- **J. Completeness. — RESOLVED.** All card slots/effects filled (2026-06-28); a **provisional** legendary-name set assigned 2026-06-30 (names in each deck file's legendary header; 2–3 alternates each in `flavor-review.md` §3). Final name flavor-lock is deferred to `docs/STATUS.md` (it does not block the build — provisional names give stable name/id). The build (#5: rebuild `cards.json` + effect registry) is unblocked; G/H tuning remains.
