...
Focused follow-up to `decks/flavor-review.md`, including consequences of the July balance review. No name below is locked unless explicitly marked in another source-of-truth document. All mechanical card candidates are inventoried in [`card-candidates.md`](card-candidates.md); this document contains only their unresolved names, species, and flavor questions.
...
- [ ] Ensure the art shows the attacker overshooting as the legendary bird folds away from the cover, so the bounce effect reads visually.
...
- [ ] Reject **Velvet Ant** for this version: the stinging females are wingless, conflicting with the newly added Flight keyword.
- [ ] Consider **Spider Wasp** if Tarantula Hawk feels too specific, but collision-check the wider Wasp naming rule first.
...
- [ ] Decide whether the STR 3 “give every other copy +3 wherever it is” design reworks the existing **Dog** or belongs to another Canine.
- [ ] If it is a separate card, choose an identity that does not violate the one-species rule.
- [ ] Art should show littermates/pack members visibly maturing with each arrival, making the wherever-they-are buffs legible.

---

## 2. Legendary naming policy

The present pool mixes original evocative names (`Eon`, `Ember`, `Vesper`) with direct literary or historical references (`Raksha`, `Falstaff`, `Carmilla`, `Scrooge`, `Methuselah`, `Lobo`). Both can be charming, but the line is currently implicit.

- [ ] Choose one policy before final flavor-lock:
  1. **Evocative-original:** names may echo literature/folklore but should not directly reuse a famous character.
  2. **Literary menagerie:** direct literary/historical names are allowed when the animal/effect fit is unusually strong.
- [ ] Recommendation: choose **Literary menagerie**, capped at roughly one conspicuous wink per deck. Falstaff and Scrooge give the game welcome humor; twenty-eight allusions would feel like a trivia set.
- [ ] Require every legendary to identify a real species in its art/flavor note, even when the printed name is abstract.
- [ ] Legendary names should pass three tests: speakable aloud, distinct at a glance, and inseparable from the card's effect after one game.
...
- [ ] Revisit **Queen Marabunta**. “Marabunta” describes an army-ant swarm more than an individual. Preferred alternative: **Queen Imperata**; softer alternative: **Queen Antonia**.
- [ ] Preserve Falstaff if the literary-menagerie policy is adopted. The tonal joke fits the unusually round bumblebee and gives the deck warmth.

### Ramp

- [ ] Strongly consider **Methuselah → Jonathan**.
  - Jonathan is a real, famous individual giant tortoise and therefore satisfies the legendary rule literally.
...
- [ ] Confirm Sirocco's animal in the art note; the name/effect are excellent, but the species should remain a real musk-producing animal rather than an invented “great musk-beast.”
...
- [ ] **Shuck** directly invokes Black Shuck more than it merely evokes it. If the stricter policy wins, rename to **Mourn** or **Grimm**.
- [ ] Avoid replacing all three at once. Lobo/Raksha/Shuck establish a coherent storybook tone if that is intentional.

---

## 4. Species and tag cleanup

- [ ] Formally allow repeated species for **Colony castes only**. Do not generalize that exception to domestic dog breeds or arbitrary subspecies.
- [ ] Pin Black Panther as a melanistic **leopard**, not jaguar.
- [ ] Make `Fish` an active tag.
- [ ] Decide whether both tortoises are Megafauna or whether both are tagless. If Jonathan remains a Ramp tortoise, Megafauna is defensible for that legendary individual; the generic Food OTK Giant Tortoise can remain tagless.
- [ ] For expansion Canines, use genuinely distinct wild species (Maned Wolf, Bat-eared Fox, Raccoon Dog, Fennec Fox, Ethiopian Wolf, Culpeo). Do not treat dog breeds as separate species.
- [ ] Treat extinct real animals such as Titanoboa and dinosaurs as animals, not invented creatures. Mark extinct status in flavor notes so the theme boundary stays explicit.
...
- [ ] **Landmarks:** keep them ecological—nests, reefs, caves, trees, carcasses, mounds—not human buildings or abstract spell objects.