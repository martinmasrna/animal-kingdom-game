...
- [ ] **TurnBot → default report pilot? BLOCKED on food_otk piloting.** Acceptance cohort ran 2026-07-03 (`greedy` vs `turn`+nb80, opp=greedy, 200/opp × 7, seed 683470156, artifact `results/bot_quality/turnbot/`): **TurnBot is a clear pilot upgrade on 5/7 decks** — aggro **+14.4%** [+11.3,+17.4], colony **+23.1%** [+20.0,+26.1], cats +6.6%, canine +6.2%, egg +5.2% (all CI>0, no collapse, mirror>50%, throughput pass) — **a wash on ramp** (−0.4% [−2.9,+2.2]), and a **decisive regression on food_otk** (−9.4% [−12.2,−6.6]: TurnBot pilots the combo *worse than 1-ply greedy*, and it's also the lone throughput failer at 12.7×). A blanket `greedy,greedy`→`turn,turn` switch would make food_otk's balance numbers *worse* than today — so **don't flip the default yet**. Single blocker = food_otk piloting (see below); the other six are ready.
  - **food_otk piloting — the blocker (NOT throughput, NOT the budget; nb=80 is byte-identical).** TurnBot's horizon stops at its own turn boundary and scores by end-of-turn eval, so it can't value food_otk's *delayed* (next-turn) payoff and dumps resources greedy wouldn't. Root-cause the underplay (candidate levers: an end-of-turn eval term that credits prepared/committed OTK setup; a shallow next-turn lethality lookahead for the payoff; check whether the readiness projection covers food_otk's engine). Validate via the paired benchmark against greedy on food_otk — the target is turn ≥ greedy there. Don't fix it with deck-specific weights (generalist invariant).
