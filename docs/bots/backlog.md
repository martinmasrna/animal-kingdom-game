...
- [ ] **TurnBot → default report pilot?** Throughput gate now nearly met: at 200-game gauntlet ratios **6/7 decks are ≤~8.5× (pass)** after the node budget (`TURN_MAX_SEARCH_NODES=80`, shipped) — **food_otk (13.5×) is the lone failer**, and it's also *underplayed* by TurnBot (loses to greedy), so its fix is piloting, not more truncation. Remaining before switching `./report` default `greedy,greedy`→`turn,turn`: run the acceptance winrate cohort (200/opp) and decide whether food_otk's gate miss + underplay block the switch or are an accepted single-deck caveat. Don't paper over a failed gate with deck-specific weights.
